...
void save_p_map(const PMap& map_0, const string& filename) {
    ...
    for (const auto& [key, props] : map_0) {
        ...
        for (const auto& next_s : props.previous_states) {
            p_write_state(out, next_s);
        }
    }
    out.close();
    cout << "PMap saved successfully to " << filename << endl;
}

PMap load_p_map(const string& filename) {
    PMap map_0;
    ...
    for (size_t i = 0; i < map_size; ++i) {
        ...
        props.previous_states.resize(vec_size);
        for (size_t j = 0; j < vec_size; ++j) {
            props.previous_states[j] = p_read_state(in);
        }
