...
State_str lr(const State_tuple& state)
{
    State_str lr_next_state_str = {0};
    Player attacker;
    Player victim;
    ...
    if (get<1>(victim) == 0) return lr_next_state_str;
    // if attacking hand or victim hand is 0
    if (get<0>(attacker) == 0 || get<1>(victim) == 0) return lr_next_state_str;
    get<1>(victim) = static_cast<short>((get<1>(victim)+get<0>(attacker))%MAX_FINGER);
