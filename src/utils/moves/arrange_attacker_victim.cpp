...
State_str arrange_state(const State_tuple& og_state, const Player& n_victim)
{
    ...
    if (og_attacker_index == 0)
    {
        ...
        get<0>(next_state_tuple) = og_attacker;
        get<1>(next_state_tuple) = n_victim;
    ...
    get<0>(next_state_tuple) = n_victim;
    get<1>(next_state_tuple) = og_attacker;
    get<2>(next_state_tuple) = static_cast<short>(og_attacker_index^1);
    return tuple_to_str(next_state_tuple);
}
