...
State_str rr(const State_tuple& state)
{
    State_str rr_next_state_str = {0};
    Player attacker;
    Player victim;
    ...
    if (get<1>(victim) == 0) return rr_next_state_str;
    // if attacking hand or victim hand is 0
    if (get<1>(attacker) == 0 || get<1>(victim) == 0) return rr_next_state_str;
    get<1>(victim) = static_cast<short>((get<1>(victim)+get<1>(attacker))%MAX_FINGER);
